...
<link href="../../style.css" rel="stylesheet" type="text/css">
 <script type="text/javascript" src="https://www.gstatic.com/charts/loader.js"></script><%-- chart관련 스크립트 받아옴. --%>
    <script type="text/javascript">
      google.charts.load('current', {'packages':['line']}); <%-- 구글 차트에서 line형태의 패키지를 연결함 --%>
      ...
      function drawChart() {
   			...
   			if(Month.length()==1){Month = "0"+Month;} //만약 현재 달의 자릿수가 1일 경우 0을 앞에 추가해줌!! DB의 저장된 값은 01,02...형식이고 캘린더는 1,2...로 받기 때문에 수정해줘야함.
				...
				//위에 달은 31일까지 있으므로 31번 for문을 돌림
	  	 	 	...
	  	 	 	%>
	   		 		data.addRows([<%-- data에 행을 추가함. 총 31번 --%>  
	         			['<%=i %>', <%=cb.Chart_days(c_date)%>, <%=cb.getDays(date)%>]<%-- i는 1~31일, cb.Chart_days(c_date)는 c_date값을 가지고 Chart_days로 감. --%>
	         		]);
  	 	 	<% }} else if(Month.equals("04")||Month.equals("06")||Month.equals("09")||Month.equals("11")){
  	 			for(int i=1; i<=30; i++){
	  	 	 	...
	  	 	 	%>
	   		 		data.addRows([             
	         			['<%=i %>', <%=cb.Chart_days(c_date)%>, <%=cb.getDays(date)%>]
	         		]);
	   		<%}} else {
  	 			for(int i=1; i<=28; i++){
  	 				String day = Integer.toString(i);
        ...
        <%-- div element를 parameter로 전달함. curve_chart은 자료로써 마우스를 차트자료에 올렸을 때 자료가 출력됨. --%>
        <%-- Line()은 라인형태의 chart형식을 나타냄. --%>
